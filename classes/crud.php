...
//Counting All
	...
	public function insertTopic($post)
	{
		...
		$lname=$this->cleanse($post['lname']);
		$sname=$this->cleanse($post['sname']);
		$smatno=$this->cleanse($post['smatno']);
		$session=$this->cleanse($post['session']);
		$department=$this->cleanse($post['department']);

		$query="insert into topic(name,info,lname,sname,smatno,session,department) values('$name','$info','$lname','$sname','$smatno','$session','$department')";
		$sql = $this->con->query($query);
		if ($sql==true) {
			header("Location:view-topic.php?msg=Topic Data was created successfully&type=success");
		}else{
